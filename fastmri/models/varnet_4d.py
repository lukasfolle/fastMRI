...
class NormUnet(nn.Module):
    ...
    def complex_to_chan_dim(self, x: torch.Tensor) -> torch.Tensor:
        ...
        assert two == 2
        return x.permute(0, 6, 1, 2, 3, 4, 5).reshape(b, 2 * c, o, d, h, w)

    def chan_complex_to_last_dim(self, x: torch.Tensor) -> torch.Tensor:
        b, c2, o, d, h, w = x.shape
        assert c2 % 2 == 0
        c = c2 // 2
        return x.view(b, 2, c, o, d, h, w).permute(0, 2, 3, 4, 5, 6, 1).contiguous()
...
class SensitivityModel(nn.Module):
    ...
    def chans_to_batch_dim(self, x: torch.Tensor) -> Tuple[torch.Tensor, int]:
        b, c, o, d, h, w, comp = x.shape

        return x.view(b * c, 1, o, d, h, w, comp), b

    def batch_chans_to_chan_dim(self, x: torch.Tensor, batch_size: int) -> torch.Tensor:
        bc, _, o, d, h, w, comp = x.shape
        c = bc // batch_size

        return x.view(batch_size, c, o, d, h, w, comp)
    ...
    def get_pad_and_num_low_freqs(
        ...
        if num_low_frequencies is None or num_low_frequencies == 0:
            ...
            squeezed_mask = mask[:, 0, 0, 0, 0, :, 0].to(torch.int8)
...
class VarNetBlock(nn.Module):
    ...
    def sens_expand(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
        return fastmri.fft3c_new_offsets(fastmri.complex_mul(x, sens_maps))

    def sens_reduce(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
        return fastmri.complex_mul(
            fastmri.ifft3c_new_offsets(x), fastmri.complex_conj(sens_maps)
    ...
    def forward(
        ...
        zero = torch.zeros(1, 1, 1, 1, 1, 1).to(current_kspace)
