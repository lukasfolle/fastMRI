...
from torch.nn import Conv1d, ConvTranspose1d, InstanceNorm3d
...
class Unet1D(nn.Module):
    ...
    def __init__(
        ...
        self.downsample_conv_layers = nn.ModuleList([Conv1dMod(chans, chans, kernel_size=1, stride=2)])
        ch = chans
        for _ in range(num_pool_layers - 1):
            self.down_sample_layers.append(ConvBlock(ch, ch * 2, drop_prob))
            ch *= 2
            self.downsample_conv_layers.append(Conv1dMod(ch, ch, kernel_size=1, stride=2))
        ...
        self.up_conv.append(
            nn.Sequential(
                ...
                Conv1dMod(ch, self.out_chans, kernel_size=1, stride=1),
            )
        )
...
class Conv1dMod(Conv1d):
    ...
    def batch_offset_to_third_dim(self, x: torch.Tensor) -> torch.Tensor:
        bdhw, c, o = x.shape
        return x.view(self.b, self.d, self.h, self.w, c, o).permute(0, 4, 5, 1, 2, 3).contiguous()
...
class ConvTranspose1dMod(ConvTranspose1d):
    def offset_to_batch_dim(self, x: torch.Tensor) -> torch.Tensor:
        b, c, o, d, h, w = x.shape
        self.b = b
        self.d = d
...
if __name__ == "__main__":
    unet = Unet1D(1, 1, chans=16, num_pool_layers=3)  # 367k parameters
    print(f"Number of parameters: ({sum(p.numel() for p in unet.parameters() if p.requires_grad)})")
    ret = unet(torch.rand((15, 1, 8, 16, 16, 16)))
    print(ret.shape)
